...
This document describes the Python tool layer available to the AI model for interacting with the sandboxed filesystem.
...
```
Path.iterdir() -> returns directory entries with type info
```
...
```
Path.read_text(encoding="utf-8") -> returns file content as string
```
...
```
Path.write_text(content, encoding="utf-8") -> creates/overwrites file
```
...
```
Path.unlink() -> removes file from filesystem
```
...
```
Path.mkdir(parents=True, exist_ok=True) -> creates directory tree
```
...
```
Path.stat() -> returns filesystem metadata
```
...
- `No such file or directory` — file/directory doesn't exist
- `Not a directory` — tried to list a file as directory
- `Is a directory` — tried to read/delete directory as file

---

## Security

All paths are validated through `resolve_sandbox_path()` which:
